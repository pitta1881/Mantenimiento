...
export default function loadOrdenesDeTrabajo(config, datos, start, end, vista) {
    let dataJson;
    let endAsDate;
    if (vista == 'dia') {
        endAsDate = new Date(end[0], end[1] - 1, end[2], 0, 0, 0, 0);
        dataJson = loadOrdenesDeTrabajoDia(datos, start, endAsDate);
    } else if (vista == 'mes') {
        endAsDate = new Date(end[0], end[1] - 1, 31, 0, 0, 0, 0);
        dataJson = loadOrdenesDeTrabajoMes(datos, start, endAsDate);
    } else {
        endAsDate = new Date(end[0], end[1] - 1, 31, 0, 0, 0, 0);
        dataJson = loadOrdenesDeTrabajoAnio(datos, start, endAsDate);
    }
...
function loadOrdenesDeTrabajoDia(datos, start, end) {
    let labelsDays = [];
    let day;
    let month;
    let year;
...
function loadOrdenesDeTrabajoMes(datos, start, end) {
    let labelsMonths = [];
    let month;
    let year;
    let dMonths = start;
...
function loadOrdenesDeTrabajoAnio(datos, start, end) {
    let labelsYears = [];
    let year;
    let dYear = start;
    do {
