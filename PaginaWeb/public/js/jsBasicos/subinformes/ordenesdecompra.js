...
export default function loadOrdenesDeCompra(config, datos, start, end, vista) {
    let dataJson;
    let endAsDate;
    if (vista == 'dia') {
        endAsDate = new Date(end[0], end[1] - 1, end[2], 0, 0, 0, 0);
        dataJson = loadOrdenesDeCompraDia(datos, start, endAsDate);
    } else if (vista == 'mes') {
        endAsDate = new Date(end[0], end[1] - 1, 31, 0, 0, 0, 0);
        dataJson = loadOrdenesDeCompraMes(datos, start, endAsDate);
    } else {
        endAsDate = new Date(end[0], end[1] - 1, 31, 0, 0, 0, 0);
        dataJson = loadOrdenesDeCompraAnio(datos, start, endAsDate);
    }
...
function loadOrdenesDeCompraDia(datos, start, end) {
    let labelsDays = [];
    let day;
    let month;
    let year;
...
function loadOrdenesDeCompraMes(datos, start, end) {
    let labelsMonths = [];
    let month;
    let year;
    let dMonths = start;
...
function loadOrdenesDeCompraAnio(datos, start, end) {
    let labelsYears = [];
    let year;
    let dYear = start;
    do {
